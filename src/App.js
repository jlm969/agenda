...
function App() {
  ...
  const [patients, setPatients] = useState(() => {
    ...
    try {
      ...
      return Array.isArray(parsedPatients) ? parsedPatients.map(p => ({
        ...p,
        // Aseguramos que el campo historiaClinica exista
        historiaClinica: p.historiaClinica || "" 
      })) : [];
    } catch (e) {
      console.error("Error parsing saved patients from localStorage", e);
      return [];
    }
  });
  ...
  // Nueva función para actualizar un paciente (usada para el historial clínico)
  const updatePatient = (index, updatedData) => {
    const updatedPatients = patients.map((patient, i) => i === index ? { ...patient, ...updatedData } : patient);
    setPatients(updatedPatients);
  };
